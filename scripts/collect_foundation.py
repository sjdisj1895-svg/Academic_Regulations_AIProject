...
def parse_toc(paras):
    ...
    for i, t in enumerate(paras[:800]):  # 목차는 문서 앞부분에 존재
        ...
        if m and re.search(r"(정관|규정|지침|예규|세칙|요령|규칙|법|시행령)\s*(\[[^\]]*\])?\s*$",
            ...
            # 셀을 문단 단위로 순서대로 뽑아오면서 이름 바로 다음 문단이 시작쪽수, 그다음이 "~",
            # 그다음이 끝쪽수가 된다. 이 쪽수는 문서 자체가 "인쇄한" 번호라 조항 하나하나의 정확한
            # 위치까지는 알 수 없지만(그건 문서 어디에도 없음), 사용자가 실제로 문서를 읽을 때
            # 페이지 하단에서 보는 번호와 정확히 일치한다 — 이 규정이 몇~몇 쪽 사이에 있는지
            # 범위로 안내하는 데 쓴다.
            page_start = page_end = None
            if (i + 3 < len(paras) and re.match(r"^\d+$", paras[i + 1])
                    and paras[i + 2] in ("~", "∼", "〜", "～")
                    and re.match(r"^\d+$", paras[i + 3])):
                page_start, page_end = int(paras[i + 1]), int(paras[i + 3])
            ...
            for j in range(i + 1, min(i + 6, len(paras))):
                if re.match(r"^[가-힣]+(과|실|센터|단)$", paras[j]):
                    dept = paras[j]
                    break
            toc.append({"no": int(m.group(1)), "name": name, "part": part,
                        "department": dept, "page_start": page_start, "page_end": page_end})
            toc_end = i
    return toc, toc_end
    ...
    try:
        ...
        for rule in rules:
            ...
            results.append({
                ...
                "law_url": "", "rulebook_page": rule.get("page_start"),
                "rulebook_page_end": rule.get("page_end"),
