...
### 4-1. 학적(휴학·복학·전과·수강·성적·졸업) 질문 전용 점검

학생·교직원이 가장 많이 묻는 학적 질문 23개만 따로 모아 점검하는 명령입니다.
(서버를 켜 두지 않아도 실행됩니다)

```powershell
python scripts/evaluate_academic.py          # 검색 단계만 점검 — 1~2분
python scripts/evaluate_academic.py --llm    # AI 답변까지 점검 — 외부 API 사용 시 수 분
```

- 결과 예시:

  ```
  [요약] 검색 단계: 정답 조항 상위 5건 내 포함 23/23 (100.0%)
  [요약] 부설학교 학칙이 1위인 문항: 0건 (0건이 이상적)
  ```

- "부설학교 학칙이 1위인 문항"은 사범대학부설고등학교·중학교 학칙이 대학 학부 질문의
  1위로 잘못 올라온 횟수입니다. 0건이 정상이며, 늘어나면 개발 담당자에게 알려주세요.
- 결과는 `data/academic_quality_report.json`에 저장되며, 각 질문의 상위 5건 조항이
  함께 기록되므로 "어느 조항이 잘못 나왔는지"를 바로 확인할 수 있습니다.
- 학적 질문에 자주 쓰는 말과 규정 원문 용어가 다를 때(예: "자퇴" ↔ 규정은 "퇴학")는
  `scripts/search_engine.py`의 `QUERY_SYNONYMS` 목록에 한 줄 추가하면 됩니다.
