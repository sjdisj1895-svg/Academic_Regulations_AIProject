...
## 0. 한눈에 보기 — 이 시스템은 무엇이고, 지금 어디까지 왔나

### 무엇을 하는 시스템인가

경상국립대학교의 **대학 규정(학칙·규정·지침)과 산학협력단 규정 426건**을 한 검색창에서
**조·항 단위**로 찾아주고, 담당부서·연락처와 시행일까지 한 번에 보여주는 서비스입니다.
두 가지 방식으로 쓸 수 있습니다.

| 탭 | 이런 때 쓰세요 | 예 | 걸리는 시간 |
|---|---|---|---|
| 🔍 **검색** | 단어나 짧은 문장으로 관련 조항을 빠르게 찾을 때 | "휴학", "연구비 신청" | 0.1초 안팎 |
| 💬 **AI에게 질문하기** *(현재 비공개 — 6장 스위치로 오픈)* | 말하듯 물어보고 답변 문장으로 정리받고 싶을 때 | "휴학 신청은 언제까지 해야 해?" | 수 초 (근거 조항 함께 표시) |

> AI 답변은 **항상 규정 원문에서 찾은 조항만 근거로** 만들고, 근거가 없으면 "규정에서
> 찾을 수 없습니다"라고 정직하게 답하도록 여러 겹의 안전장치가 있습니다.
> 그래도 참고용이며, **법적 효력은 원본 규정을 따릅니다.**

### 지금 어디서 볼 수 있나

| 구분 | 주소 | 비고 |
|---|---|---|
| **운영(학내망)** | `https://gapps.gnu.ac.kr/regulation/` | 기존 주간업무보고 서버(nginx)와 80/443 포트를 함께 사용. 현재 정보전산처만 접속 가능 |
| 운영(직접 접속) | `http://gapps.gnu.ac.kr:8000` | 위 주소와 같은 서버. 위 주소가 정착되면 닫을 예정 |
| 개발용(내 PC) | `http://127.0.0.1:8000/` | 2번 항목대로 직접 실행했을 때 |

> 💡 검색 결과 화면의 **주소창 URL을 그대로 복사**하면 검색어·출처·카테고리·정렬 조건까지 함께 공유됩니다
> (예: `…/regulation/?q=휴학&source=대학&sort=date`). 같은 질문을 다시 하면 결과가 캐시에서 즉시 나옵니다.

### 화면 사용 팁 (검색 탭)

| 기능 | 어디서 | 설명 |
|---|---|---|
| 출처 | 검색창 아래 "전체 / 대학 / 산학협력단" | **여러 개 동시 선택** 가능. 누르면 그 출처의 카테고리만 아래 줄에 나타나고, 둘 다 고르면 '전체'로 정리됩니다 |
| 카테고리 | 점선 아래 한 줄 (학칙·규정·지침 / 제1편~제7편) | **여러 개 동시 선택** 가능. 누르면 선택, 다시 누르면 해제 |
| 출처 균형 | 자동 | 출처를 고르지 않은 '전체' 검색에서는 대학·산학협력단 각각 **최소 3건**이 결과에 포함되도록 보정됩니다 (대학 규정이 훨씬 많아 산학협력단 결과가 묻히는 것을 방지) |
| **필터 더보기** | 검색창 아래 오른쪽 ☰ 버튼 | 아래 세 가지와 키보드 단축키 안내가 이 안에 있습니다 |
| ㄴ 정렬 | 필터 더보기 안 | **관련도순**(기본) ↔ **시행일 최신순**. 최신순은 관련도로 고른 결과 안에서만 순서를 바꿔, 무관한 최신 규정이 끼어들지 않습니다 |
| ㄴ 최근 1년 개정만 | 필터 더보기 안 | 최근 1년 안에 시행·개정된 규정만 봅니다 (개정 사항 점검에 유용) |
| ㄴ 폐지된 규정도 포함 | 필터 더보기 안 | 기본은 현행 규정만. 개정 연혁 확인 시에만 켭니다 |
| 적용 중 태그 | 필터 영역 맨 아래 "적용 중 …" | 지금 걸려 있는 조건이 파란 태그로 표시됩니다. **×** 로 하나씩, "모두 해제"로 전부 풉니다 |
| 결과 묶음 | 결과 목록 | 같은 규정의 조항은 한 상자(규정명 · 출처 · N개 조항)로 묶여 나옵니다. 좌측 색 막대: 파랑=대학, 금색=산학협력단, 빨강=폐지 |
| 미리보기 | 카드의 "미리보기" 또는 카드 클릭 | PC에서는 **오른쪽 패널**로 열려 목록을 보면서 읽을 수 있고, 열린 채 다른 카드를 누르면 내용이 바뀝니다(지금 보는 카드는 파란 배경). 휴대폰은 전체 창으로 열립니다. `Esc`로 닫기 |
| 인쇄 | 미리보기 창의 🖨 인쇄 (또는 Ctrl+P) | 규정명·시행일·조항·담당부서·본문만 깔끔하게 출력됩니다 |
| 키보드 | 어디서나 | `/` 검색창으로 · `↑` `↓` 결과 카드 이동 · `Enter` 미리보기 · `Esc` 닫기 |
| 맨 위로 | 화면 우하단 ↑ 버튼 | 결과를 내려 보다가 검색창으로 돌아갈 때 (300px 이상 스크롤하면 나타남) |
| 처음으로 | 상단 로고 또는 제목 클릭 | 검색어·필터·정렬을 모두 지우고 첫 화면으로. 실수로 눌렀으면 브라우저 뒤로가기로 직전 검색 복귀 |

### 지금까지 만든 것 (2026-09 기준, 개발 단계 T1~T20)

1. **검색 시스템 (T1~T5)** — 규정 원문 자동 수집 → 조·항 단위로 쪼개기(9,258개) →
   한국어 AI가 의미를 숫자로 바꿔 저장(벡터DB) → 키워드+의미 결합 검색 → 웹 화면.
   자주 쓰는 검색어 15개 자동 점검 100%, 응답 0.05초.
2. **AI에게 질문하기 (T6~T10)** — 검색으로 찾은 조항만 근거로 답변 문장 생성. 근거 없는
   규정명·숫자를 지어내면 자동 경고, 무관한 질문은 AI를 부르지 않고 즉시 거절.
   질문·답변 로그와 비용 집계 스크립트 포함.
3. **정확도 개선 여정 (T11~T15)** — 더 좋은 AI 임베딩 모델로 두 번 바꿔봤지만(BGE-M3,
   e5-large) 둘 다 "무관한 질문도 관련 있다고 판단"하는 부작용이 있어 되돌렸고, 대신
   ① 상위 후보만 정밀 재비교(재순위화), ② 조문에 소속 장(章) 제목 부여, ③ 질문형
   질의는 의미 검색 비중을 높이고, ④ 질문을 규정 문체로 바꿔 검색하는 방식으로 올렸습니다.
   운영용 AI는 학교 FactChat API를 통한 **Claude Haiku 4.5**로 바꿔 답변이 수십 초 → 수 초.
4. **운영 배포 (T16)** — 리눅스 운영 서버에 올리고, 업무보고 페이지와 포트를 나눠 쓰도록
   nginx 경로(`/regulation/`)로 연결.
5. **화면·데이터 다듬기 (T17~T20)** — 카테고리를 대학/산학협력단으로 나눠 표시, 담당부서
   표기 통일, 경상국립대 공식 VI(전용색상·시그니처) 적용, **시행일·개정일 표시**,
   **폐지된 규정 32건을 기본 제외**(토글로 포함 가능), 학적(휴학·복학·졸업 등) 질문 23개
   전용 점검 세트.

> 🖼 글보다 그림이 편하면 [`DIAGRAMS.md`](DIAGRAMS.md)를 보세요 — 전체 구성도, 데이터 준비·검색·AI 답변
> 순서도, "문제가 생겼을 때 어디를 보나" 의사결정 순서도가 있습니다.

### 담당자가 평소에 할 일은 세 가지뿐

| 언제 | 무엇을 | 어디 보나 |
|---|---|---|
| 규정이 바뀌었을 때 | `python scripts/refresh_all.py` 한 줄 (수집→시행일 보강→청킹→벡터DB→점검 자동) | 3장 |
| 가끔(월 1회 정도) | 품질 점검 스크립트 3개 실행해 100%인지 확인 | 4장 |
| 답변이 이상하다는 말을 들었을 때 | 근거 조항 확인 → 로그 확인 → 개발 담당자에게 전달 | 6-3, 6-4 |
...
> 실제 운영은 리눅스 서버(`https://gapps.gnu.ac.kr/regulation/`)에서 systemd로 항상 켜져 있으며,
> 그 서버에서의 실행·재시작 방법은 9장을 참고하세요. 이 2장은 내 PC에서 확인용으로 켤 때의 방법입니다.
...
### 4-2. 시행일·개정일과 "폐지된 규정" 처리

검색 결과 카드 오른쪽 위에 **시행 2026-05-29 · 일부개정**처럼 시행일과 개정 종류가
표시되고, 미리보기에는 규정 번호(예: 경상국립대학교학칙 제529호)까지 나옵니다.

- **폐지된 규정은 기본적으로 검색 결과와 AI 답변 근거에서 제외**됩니다. (2026-09 기준
  426건 중 32건이 폐지 상태 — 장학위원회 규정, 발전기금재단 연구비 지원 지침 등.
  이전에는 이들이 현행 규정처럼 검색돼 잘못된 안내로 이어질 수 있었습니다)
- 개정 연혁 확인 등으로 폐지 규정을 봐야 할 때는 필터 아래 **"폐지된 규정도 포함"** 을
  켜세요. 폐지 규정은 빨간 **폐지** 배지와 연한 붉은 배경으로 구분됩니다.
  검색어에 "폐지"가 들어 있으면 토글을 켜지 않아도 자동으로 포함됩니다.
- 이 정보는 `python scripts/enrich_dates.py`가 국가법령정보센터 원문 헤더
  (`[시행 2026.5.29.] [경상국립대학교학칙 제529호, 2026.5.29., 일부개정]`)를 읽어
  `data/regulations.json`에 채워 넣습니다(약 3~4분, 벡터DB 재구축 불필요).
  `refresh_all.py`로 갱신하면 자동으로 함께 실행되므로 따로 신경 쓸 필요는 없습니다.
- 산학협력단 규정은 법령정보센터에 없어, 규정집 원문의 "제정 2004.01.30 / 개정
  2025.07.01" 이력 줄에서 마지막 날짜를 최종 개정일로 씁니다.

### 4-3. 결과 순위에 적용되는 자동 보정 규칙 (알아두기)

검색 결과에는 아래 보정이 자동으로 들어갑니다. "왜 이 조항이 위에/아래에 나오지?"가
궁금할 때 참고하세요. 모두 **제외가 아니라 감점**이라 필요한 경우엔 여전히 검색됩니다.

| 규칙 | 조건 | 효과 |
|---|---|---|
| 대학·산학협력단 **공통 규정 병합** | 같은 규정이 양쪽 규정집에 있을 때(17개) | 같은 조문은 한 장으로만 표시 + "대학·산학협력단 공통" 배지 |
| 부칙·별표·별지 감점 | 검색어에 부칙/별표/서식/시행일 같은 말이 없을 때 | 본문 조항이 먼저, 부칙("공포한 날부터 시행")은 뒤로 |
| 대학원 규정 감점 | 검색어에 대학원/석사/박사/학위논문 등이 없을 때 | 학부 규정이 먼저 (대학원 질문이면 감점 없음) |
| 부설고·중학교 학칙 감점 | 검색어에 부설/고등학교/중학교가 없을 때 | 대학 학칙·학사관리 규정이 먼저 |
| 폐지 규정 제외 | "폐지된 규정도 포함" 토글이 꺼져 있고 검색어에 '폐지'가 없을 때 | 결과·AI 근거에서 제외 |
...
> 🔒 **현재 상태: 화면에서 숨김 (기능은 준비 완료).** 정식 오픈 전까지 "AI에게 질문하기" 탭이 화면에
> 보이지 않습니다. 서버 코드·API는 그대로 살아 있으며, **환경변수 한 줄로 켤 수 있습니다.**
>
> | 켜기 | 서버의 systemd 서비스 파일 `Environment=` 항목에 `GNU_AI_TAB_ENABLED=1` 추가 → `sudo systemctl daemon-reload` → `sudo systemctl restart gnu-regulation-search` |
> |---|---|
> | 끄기(기본) | 위 항목을 지우거나 `0`으로 → 재시작 |
> | 확인 | 브라우저 새로고침 후 검색창 위에 "🔍 검색 / 💬 AI에게 질문하기" 탭이 보이면 켜진 상태. `…/api/filters`의 `ai_tab_enabled` 값으로도 확인 가능 |
>
> 내 PC에서 테스트할 때는 실행 전 `$env:GNU_AI_TAB_ENABLED = "1"`을 설정하면 됩니다.
...
### 6-1. 실행 방법 — 어떤 AI가 답을 만드나

"AI에게 질문하기" 기능은 **2번 항목의 서버 실행 명령을 그대로 사용**하며, 별도로 켜고 끄는
스위치가 없습니다. 서버를 실행하면 검색 기능과 AI 질문 기능이 함께 켜집니다.

답변 문장을 만드는 AI는 두 가지 중 하나가 자동으로 선택됩니다.

| 구분 | 어떤 AI | 답변 시간 | 언제 쓰이나 |
|---|---|---|---|
| **운영 표준** | 학교 FactChat API를 통한 **Claude Haiku 4.5** (외부 API) | **수 초** | 6-2의 환경변수(API 키 등)가 설정된 서버 — 현재 운영 서버 |
| 대체(백업) | 컴퓨터 안에서 무료로 도는 Qwen2.5-1.5B (로컬 AI) | 20~80초 (첫 질문은 60초 넘을 수도) | API 키가 없는 PC, 또는 외부 API 장애 시 |

- 어느 쪽이 쓰이는지는 서버 시작 로그의 `[RAG] 외부 API 백엔드 사용: ...` 문구로 알 수 있습니다
  (이 문구가 없으면 로컬 AI로 동작 중).
- 어느 AI든 **검색으로 찾은 조항만 근거로** 답을 만들고, 근거가 없으면 "규정에서 찾을 수
  없습니다"라고 답합니다. 검색 자체는 0.1초 내외로 빠르고, 시간이 걸리는 건 "문장을 다듬는"
  AI 부분과 상위 후보를 정밀 재비교하는 단계(약 6초, 후보 10개 기준 — `GNU_RERANK_POOL`로 조정)입니다.
- 폐지된 규정은 AI 답변의 근거에서도 자동으로 제외됩니다(4-2 참고).
- **답변은 실시간으로 채워집니다**: 근거 조항 카드가 먼저 뜨고(검색 완료, 수 초), 답변 문장이 글자 단위로
  나타난 뒤 마지막에 검증(규정명·숫자 확인, 출처 첨부)을 거친 최종 문장으로 바뀝니다. 로컬 AI에서는
  완성된 답이 한 번에 나타나며, 실시간 전송이 안 되는 네트워크에서는 자동으로 기존 방식으로 동작합니다.
- **이어서 물어볼 수 있습니다**: "휴학 신청은 언제까지 해야 해?" 뒤에 "그럼 복학은?"처럼 짧게 물어도
  직전 대화(최근 3개)를 참고해 알아듣습니다. 단, 답변의 근거는 여전히 새로 검색한 규정 조항만 사용합니다.
  화면을 새로 고치면 대화 맥락은 초기화됩니다.
